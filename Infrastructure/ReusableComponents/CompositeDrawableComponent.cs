...
namespace Infrastructure.ReusableComponents
{
        ...
        where ComponentType : IGameComponent
    {
        ...
        protected List<Sprite2D> m_Sprites = new List<Sprite2D>();
        ...
        protected virtual void OnComponentRemoved(GameComponentEventArgs<ComponentType> e)
        {
            ...
            Sprite2D sprite = e.GameComponent as Sprite2D;
            if (sprite != null)
            {
        ...
        private void childDrawOrderChanged(object sender, EventArgs e)
        {
            ...
            Sprite2D sprite = sender as Sprite2D;
            if (sprite != null)
            {
                m_Sprites.Remove(sprite);
            }
        ...
        {
            Sprite2D sprite = i_Drawable as Sprite2D;
            if (sprite != null)
            {
                int idx = m_Sprites.BinarySearch(sprite, DrawableComparer<Sprite2D>.Default);
                if (idx < 0)
                {
                    idx = ~idx;
                }
        ...
        {
            if (i_Component is Sprite2D)
            {
                (i_Component as Sprite2D).SpriteBatch = m_SpriteBatch;
            }
